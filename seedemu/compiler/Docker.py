...
DefaultImages.append(DockerImage('ubuntu:latest', []))
...
class Docker(Compiler):
    ...
    def _selectImageFor(self, node: Node) -> Tuple[DockerImage, Set[str]]:
        ...
        if self.__disable_images:
            ...
            (image, _) = self.__images['ubuntu:latest']
    ...
    def _compileNode(self, node: Node) -> str:
        ...
        # docker apt 换源
        dockerfile += 'RUN sed -i s/archive.ubuntu.com/mirrors.aliyun.com/g /etc/apt/sources.list && sed -i s/security.ubuntu.com/mirrors.aliyun.com/g /etc/apt/sources.list \n'
        

        # Using douban pipy mirror
        # dockerfile += 'RUN pip3 install -i https://pypi.douban.com/simple/ -U pip\n'
        # dockerfile += 'RUN pip3 config set global.index-url https://pypi.douban.com/simple/\n'
